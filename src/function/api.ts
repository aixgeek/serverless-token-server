...
export class ApiHandler {
    ...
    @Validate()
    @Get('/realm')
    async listRealm(@Query() query: { page: number, pageSize: number, keyword?: string }) {
    ...
    @Validate()
    @Get('/secret')
    async listSecret(@Query() query: { page: number, pageSize: number, realm?: string }) {
    ...
    async getSecret(@Param('key') key: string) {
        const entity = await this.secretRepo.findOne({ where: { key: Equal(key) } })
        if (!entity) {
